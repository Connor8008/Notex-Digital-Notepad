...
// Isolated User Dashboard Component
function DashboardWrapper({ 
  ...
  // Load and store isolated workspace data keyed by the logged-in user email
  const [notes, setNotes] = useState<NoteItem[]>(() => {
    const saved = localStorage.getItem(`notex_notes_archive_${loggedInEmail}`);
    if (saved) return JSON.parse(saved);
    
    // Seed default notes if it's the demo account
    if (loggedInEmail === 'curator@notex.archive') {
      return INITIAL_NOTES;
    }
    
    // Seed a welcome note card for new registered accounts
  ...
  // Active note states for modal interaction
  const [selectedNote, setSelectedNote] = useState<NoteItem | null>(null);
  const [isNoteModalOpen, setIsNoteModalOpen] = useState<boolean>(false);

  // Write variables to user-keyed localStorage when state changes
  useEffect(() => {
  ...
  // Switch renderer inside tab dashboard
  ...
  const handleTabChange = (tab: AppTab) => {
    setActiveTab(tab);
    setIsSidebarOpen(false); // Close mobile drawer when switching tabs
  };
  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-black text-white' : 'bg-white text-black'} antialiased overflow-x-hidden selection:bg-neutral-200 dark:selection:bg-neutral-800 relative transition-all duration-300 ${getSystemFontClass()}`}>
      ...
      {/* Slide-out mobile side drawer backdrop */}
      <AnimatePresence>
        {isSidebarOpen && (
          <>
            <motion.div
      ...
      {/* Main dashboard content body offset by fixed navigation headers */}
      ...
      {/* Note Reader or Ink-Editor Modal sheet */}
      <AnimatePresence>
        {isNoteModalOpen && (
          <NoteModal
...
export default function App() {
  ...
  // If not signed in, show the togglable Registration / Login form
  if (!loggedInEmail) {
    return <LoginForm onLogin={handleLogin} />;
  }

  // Keying DashboardWrapper by loggedInEmail destroys previous user state instantly on switch/logout
  return (
    <DashboardWrapper 
      key={loggedInEmail}
