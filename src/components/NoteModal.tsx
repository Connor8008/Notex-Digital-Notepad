...
export default function NoteModal({
  ...
  // Zen Hourglass States & Focus Timer
  ...
  // Sync content from rich editor to state
  const handleInput = () => {
  ...
  // Check active styles depending on cursor position / command states (MS Word style)
  ...
  // Helper to place cursor inside a span or element
  ...
  // Custom Font Size selection logic applying to the ENTIRE editor canvas
  const handleFontSizeChange = (size: string) => {
    setEditorFontSize(size);
    if (editorRef.current) {
      editorRef.current.style.fontSize = size;
      // Strip any inline span font sizes inside the content to ensure absolute consistency
  ...
  // Custom Font Family selection logic applying to the ENTIRE editor canvas
  const handleFontFamilyChange = (font: string) => {
    ...
    if (editorRef.current) {
      ...
      // Strip inline span font families to keep it completely unified
      const spans = editorRef.current.getElementsByTagName('span');
      for (let i = 0; i < spans.length; i++) {
  ...
  // Safe Multi-color Hilite Highlight selection
  const handleHighlight = (color: string) => {
    if (editorRef.current && document.activeElement !== editorRef.current) {
      editorRef.current.focus();
    }
    
    // Execute highlighter command
  ...
  // Intercept rich-pasting to enforce clean plain-text pasting, removing weird black background blocks
  ...
  // MS Word style live Find selection inside contentEditable
  ...
  // Preview Mode with search highlights
  const getRenderedPreview = () => {
    ...
    // Highlight doc search query matching elements
  ...
  // Handle innerHTML sync once when modal is fully initialized
  useEffect(() => {
  ...
  // Keep a ref to latest state to avoid clearing interval on every keystroke
  ...
  // Setup the auto-save interval of 30 seconds
  useEffect(() => {
    if (!isOpen) return;
  ...
  // Decide backdrop and card page textures
  const getPaperClass = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 select-none">
      ...
      {/* Hidden file input for uploading docs */}
      <input 
        type="file" 
        accept=".txt,.md" 
        ref={fileInputRef} 
      ...
      >
        ...
        {/* Content body wrapped inside custom chosen physical paper texture */}
        <div className={`flex-grow p-6 md:p-8 overflow-y-auto space-y-6 custom-scrollbar-thin ${getPaperClass()}`}>
          
          {/* Zen Hourglass Timer top overlay inside the physical page */}
          {isZenMode && (
          ...
          {/* Metadata Controls on physical page */}
          {!isZenMode && (
          ...
          {/* Immersive Text Writing Canvas responsive to chosen typeface */}
          <div className="flex-grow flex flex-col pt-2 min-h-[300px]">
            {/* Minimal Formatting Toolbar with Edit/Preview toggling tabs */}
            {!isZenMode && (
              <>
                ...
                {/* MS Word style Text Highlight marker tool */}
                <div className="relative">
                  <button
                    type="button"
                    onClick={() => {
                ...
                {/* Dropdown font size selection popup */}
                <div className="relative">
                  <button
                    type="button"
                    onClick={() => {
                ...
                >
                 ...
                 {/* Search document word highlighters */}
                <div className="h-4 w-[1px] bg-current/15 mx-1" />
                <button
                  type="button"
            ...
            {/* Slide-out search & replace input container */}
            <AnimatePresence>
              {showSearchInput && (
                <motion.div
            ...
            {/* Writing Canvas / contentEditable editor area */}
