...
export default function LibraryView({
  ...
  // Local state for dropdown filters shown in reference screens
  const [dateRangeFilter, setDateRangeFilter] = useState<string>('All Time');
  const [showDateDropdown, setShowDateDropdown] = useState(false);
  const [categoryFilter, setCategoryFilter] = useState<string>('All');
  const [showCategoryDropdown, setShowCategoryDropdown] = useState(false);

  // Group tags and categories for the dropdowns and tag panel
  const allCategories = useMemo(() => {
    const cats = new Set(notes.map(n => n.category));
    return ['All', ...Array.from(cats)];
  }, [notes]);

  // Handle active note classification filtering depending on tab (library, drafts, or archive)
  ...
  // Apply search query, category, and date filtering
  const filteredNotes = useMemo(() => {
    return tabFilteredNotes.filter(note => {
      ...
      // Date matching (mock filter logic for date ranges)
  ...
  // Identify the "Featured" manuscript or note item for display in the main wide article block
  const featuredNote = useMemo(() => {
    // Look for a Manuscript first, otherwise take the newest note
    const manuscript = filteredNotes.find(n => n.type === 'Manuscript');
    if (manuscript) return manuscript;
    return filteredNotes[0] || null;
  }, [filteredNotes]);

  // Sub-items for the split grid section (exclude the featured one if visible)
  ...
  // Quick tags list extracted dynamically from notes
  ...
  return (
    <div className="max-w-[1140px] mx-auto py-6 select-none">
      ...
      {/* Search Header & Filter Controls Section */}
      ...
      {filteredNotes.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 animate-fade-in">
          ...
          {/* Secondary architectural image contexts & Quick Tags */}
          <div className="md:col-span-4 space-y-8">
            ...
            {/* Quick Tags panel with highlight active click filters */}
